module SearchHelper
  def generate_link(page_number, is_current_page) 
    link_to(page_number, search_path(page_number, @query_params), 
      class: 'element link' + ' ' + (is_current_page ? 'current' : 'noncurrent'))
  end

  def generate_links(from, to)
    (from .. to).map{ |num| generate_link(num, false) }.join
  end
  ...
  def generate_dynamic_links_side(from, to, cutoff, left_count, right_count)
    if to - from < cutoff
      ...
      return generate_links(from, from + left_count) << content_tag('span', '...', class: 'element spacer') << generate_links(to - right_count, to)
    end
  end
  ...
  def generate_paging_links(current_page, page_count)
    ...
    result << generate_link(current_page, true)
